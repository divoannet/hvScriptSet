const addMask = {
  ...
  init(opt) {
    ...
    function getTags(text) {
      ...
      for (let field in this.changeList) {
        if (this.changeList.hasOwnProperty(field)) {
          let tags = this.changeList[field].tag.split(',');
            ...
            if (tags.hasOwnProperty(i)) {
              ...
              if (clearedText.match(pattern)) {
                postChangeList[field] = {
                  'tag': tags[i],
                  'field': this.changeList[field].class,
                  'content': text.match(pattern)[0].replace(clearPattern, ''),
                  'type': this.changeList[field].type
                };
              }
            }
          }
    ...
    function changeMaskForm(field, value) {
      ...
      switch (field) {
        ...
        case 'avatar':
          ...
          if (!checkImage(str)) {
            this.errorList[field] = 'В поле [Аватар] должна быть ссылка на картинку формата jpg, gif или png';
          } else {
            delete this.errorList[field];
            this.previewForm.querySelector(`.hv-preview-${field} img`).src = str;
          }
          break;
        case 'author':
          if (value.length > 25) {
            this.errorList[field] = 'Поле [Ник] не должно содержать больше 25 символов';
          } else {
            str = value !== '' ? value : UserLogin;
            delete this.errorList[field];
            this.previewForm.querySelector(`.hv-preview-${field}`).innerText = str;
          }
          break;
        case 'title':
          if (value.length > 50) {
            this.errorList[field] = 'Поле [Статус] не должно содержать больше 50 символов';
          } else {
            delete this.errorList[field];
        ...
        default:
          if (value.length > 999) {
            this.errorList[field] = `Поле [${this.changeList[field].title}] не должно содержать больше 999 символов`;
          } else {
            delete this.errorList[field];
            str = value || '';
            switch (this.changeList[field].type) {
              case 'text':
                delete this.errorList[field];
                this.previewForm.querySelector(`.hv-preview-${field}`).innerHTML = str.replace(/</gi, '&lt;');
                break;
              case 'bbcode':
                delete this.errorList[field];
                this.previewForm.querySelector(`.hv-preview-${field}`).innerHTML = bbcodeToHtml(str);
                break;
              default:
                if (checkHtml(str)) {
                  this.errorList[field] = `В поле [${this.changeList[field].title}] недопустимые теги`;
                } else {
                  delete this.errorList[field];
                  this.previewForm.querySelector(`.hv-preview-${field}`).innerHTML = str;
                }
            }
          }
          break;
      ...
      if (Object.keys(this.errorList).length) {
        ...
        for (let error in this.errorList) {
          if (this.errorList.hasOwnProperty(error)) {
            ...
            li.innerHTML = `<li> ! ${this.errorList[error]}</li>`;
    ...
    function fillForm(obj) {
      ...
      for (let change in this.changeList) {
        if (this.changeList.hasOwnProperty(change)) {
    ...
    function buildMaskDialog() {
      ...
      let _loop = function _loop(mask) {
        if (this.changeList.hasOwnProperty(mask)) {
          (function () {
            let li = document.createElement('div');
            li.className = 'hv-mask-field ' + mask;
            let input = void 0;
            switch (this.changeList[mask].type) {
              case 'html':
              case 'signature':
              case 'bbcode':
            ...
            input.addEventListener('blur', () => {
              ...
              if (input.value !== '' && !checkHtml(input.value)) {
                this.tmpMask[idField] = {
                  'tag': this.changeList[idField].tag.split(',')[0],
                  'value': input.value
                };
              } else {
                delete this.tmpMask[idField];
              }
            ...
            label.innerHTML += '<b>' + this.changeList[mask].title + '</b>';
            if (this.changeList[mask].description) {
              label.innerHTML += '<div class="description">' + this.changeList[mask].description + '</div>';
            }
            li.appendChild(label);
            if (this.changeList[mask].defaultCode) {
              const code = this.changeList[mask].defaultCode;
              ...
              if (typeof code === 'string') {
                ...
                templateButton.addEventListener('click', function () {
                  fillInput(input, this.changeList[mask].defaultCode);
      ...
      for (let mask in this.changeList) {
        _loop(mask);
      }
    ...
    function getMaskStorage(prevMasks) {
      ...
      let _loop2 = function _loop2(mask) {
        ...
        for (let item in this.changeList) {
          if (this.changeList.hasOwnProperty(item) && item !== 'avatar' && mymask[item]) {
            if (!checkHtml(mymask[item].value.toString())) {
              infoBlock += '<div class="' + item + '"><b>' + this.changeList[item].title + ':</b> ' +
    ...
    function saveMask() {
      if (Object.keys(this.tmpMask).length > 0) {
        let tempMask = JSON.stringify(this.tmpMask);
        if (Object.keys(prevMasks).length > 0) {
          if (!(hasMaskInSrorage(prevMasks, this.tmpMask) + 1)) {
            if (prevMasks.length >= maskLimit) {
              prevMasks.splice(0, 1);
            }
          } else {
            prevMasks.splice(hasMaskInSrorage(prevMasks, this.tmpMask), 1);
          }
        }
        prevMasks.push(JSON.stringify(this.tmpMask));
        $.post('/api.php',
          {
            method: 'storage.set',
        ...
        .fail(function() {
          this.errorList.common = 'Ошибка сохранения, попробуй ещё раз.';
          showErrors();
        });
      }
    }
        ...
        if (this.changeList.hasOwnProperty(mask)) {
          ...
          switch (mask) {
            case 'author':
              ...
              this.previewForm.appendChild(div);
              break;
            case 'title':
    ...
    function getStrMask() {
      let str = '';
      Object.keys(this.tmpMask).forEach(change => {
        str += `[${this.tmpMask[change].tag}]${this.tmpMask[change].value}[/${this.tmpMask[change].tag}]`;
      });
      return `[block=hvMaskStr]${str}[/block]`;
    }
